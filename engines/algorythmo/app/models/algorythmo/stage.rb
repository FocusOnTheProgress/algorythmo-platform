...
class Algorythmo::Stage < ApplicationRecord
  belongs_to :pipeline, class_name: 'Algorythmo::Pipeline', touch: true
  has_many :leads, class_name: 'Algorythmo::Lead',
  ...
  def rename(new_name)
    ...
    duplicate = pipeline.stages.where.not(id: id).exists?(['LOWER(name) = LOWER(?)', new_name])
