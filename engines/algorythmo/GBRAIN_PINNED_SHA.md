...
| Ollama embedding recipe: provider prefix `ollama`, reads `OLLAMA_BASE_URL` (default `http://localhost:11434/v1`), no API key required; models include `nomic-embed-text` (768 dims) | `src/core/ai/recipes/ollama.ts` | Founder directive 2026-06-05: SELF-HOSTED indexer, zero paid services, runs on our VPS. We pin `ollama:nomic-embed-text` / 768 dims. Synthesis (think) uses DeepSeek; OPENAI_API_KEY is a manual fallback. Validated end-to-end (capture + semantic search). |
