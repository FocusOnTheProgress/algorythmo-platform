...
| `app/javascript/dashboard/assets/scss/super_admin/index.scss` | import block | `// algorythmo: design-system-import` | Imports the Algorythmo design system into the super-admin pack so the operator console inherits the dark canvas + cyan-teal brand purge instead of staying light (Cinematic OS C1.1). |
| `app/javascript/dashboard/helper/themeHelper.js` | `setColorTheme` | `// algorythmo: rebrand-m0` | Dark-first boot: dashboard defaults to dark; only explicit `light` opts out (Cinematic OS C1). |
| `app/javascript/v3/App.vue` | data + `setColorTheme` + style | `// algorythmo: rebrand-m0` + `// algorythmo: design-system-import` | Dark-first auth/login shell + imports the Algorythmo design system into the v3 SCSS island (login page); link color → brand cyan-teal (Cinematic OS C1). |
| `app/views/layouts/vueapp.html.erb` | meta + body | `<%# algorythmo: rebrand-m0 %>` | `theme-color`/`TileColor` → command-room black; `<body class="dark">` + inline black bg to kill the white boot flash (Cinematic OS C1). |
