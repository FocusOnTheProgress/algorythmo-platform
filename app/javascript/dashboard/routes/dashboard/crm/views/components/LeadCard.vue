...
// Row 2 — channel mark (REF CRM). Borderless: a brand-coloured glyph + the
// lowercase channel word, exactly as the reference ("• whatsapp", "• email",
// "• instagram", "• tiktok"). No pill chrome — the colour lives on the glyph
// only, so the card reads quiet. The label keeps its capitalised i18n string in
// the DOM (WhatsApp) and renders lowercase via text-transform to match the ref.
...
.alg-lead-card__channel--branded .alg-lead-card__channel-label {
  color: color-mix(in srgb, var(--alg-channel-brand) 72%, white 28%);
}
