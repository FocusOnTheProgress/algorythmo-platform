// algorythmo: plan 0007 M2-f — HR Overview + sub-tab mocks.
...
import overviewCards from '../../mocks/sectors/hr-overview';
import subtabViews from '../../mocks/sectors/hr-subtabs';

// Overview and sub-tab deep views cover the same four HR sub-areas 1:1.
const HR_IDS = ['contratacao', 'treinamento', 'cultura', 'produtividade'];
const ALLOWED_GLYPHS = ['▲', '▼', '—'];

describe('hr overview mock — contract', () => {
  it('covers every HR sub-area exactly once', () => {
    expect(overviewCards.map(c => c.id)).toEqual(HR_IDS);
  });
...
describe('hr sub-tab mock — contract', () => {
  it('covers every HR sub-area exactly once', () => {
    expect(subtabViews.map(v => v.id)).toEqual(HR_IDS);
  });
