...
RSpec.describe Algorythmo::FeatureGate do
  ...
  describe 'ALGORYTHMO_CUT_FLAGS' do
    it 'contains exactly 13 cut surfaces' do
      expect(described_class::ALGORYTHMO_CUT_FLAGS.size).to eq(13)
    end
  ...
  describe '.feature_enabled?' do
    it 'delegates to account via Rails.cache (upstream contract preserved)' do
      ...
      allow(account).to receive(:feature_enabled?).with('algorythmo_show_captain').and_return(false)
      expect(described_class.feature_enabled?(account, 'algorythmo_show_captain')).to be false
    end

    it 'returns false for nil account (fail-closed)' do
      expect(described_class.feature_enabled?(nil, 'algorythmo_show_captain')).to be false
    end
  ...
  # CI guard: all 13 cut flags occupy positions 1–13 in algorythmo_feature_flags column.
  ...
  describe 'bigint safety — algorythmo_feature_flags positions' do
    it 'all 13 cut flags are at positions 1–13 (well within signed bigint range)' do
